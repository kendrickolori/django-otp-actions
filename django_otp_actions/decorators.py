    ...
    def decorator(func):
        ...
        def wrapper(request, *args, **kwargs):
            ...
            if not otp or not encrypted_context:
                return Response(
                    {"error": "OTP and context are required", "error_code": "MISSING_FIELDS"},
            ...
            try:
                ...
                return Response({"error": str(e), "error_code": code}, status=status.HTTP_410_GONE)
            except MaxRetriesExceededException as e:
                return Response({"error": str(e), "error_code": "MAX_RETRIES_EXCEEDED"}, status=status.HTTP_429_TOO_MANY_REQUESTS)
            except Exception:
                return Response({"error": "Internal Error", "error_code": "OTP_ERROR"}, status=status.HTTP_400_BAD_REQUEST)
